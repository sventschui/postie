...
import type { HTMLAttributes, TargetedEvent, ChangeEvent } from 'preact/compat';
...
const Sun = (props: HTMLAttributes<SVGSVGElement>) => (
...
const Moon = (props: HTMLAttributes<SVGSVGElement>) => (
  <svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" {...props}>
    <path
...
type Props = {
  ...
  onSearch: (search: Record<string, string | null>) => void;
};

const Header = ({ onSearch, search, onSetDarkMode, onSetLanguage, lang }: Props) => {
          ...
          {typing && text ? (
            <div className="searcher">
              <ol>
                ...
                >
                  <button onMouseUp={onSearchSubmit}>Search mails with text {text}</button>
                </li>
                <li
                  onMouseOver={() => {
                    setType('subject');
                ...
                >
                  <button onMouseUp={onSearchSubmit}>Search mails with subject {text}</button>
                </li>
                <li
                  onMouseOver={() => {
                    setType('to');
                ...
                >
                  <button onMouseUp={onSearchSubmit}>Search mails to {text}</button>
                </li>
              </ol>
            </div>
          ) : null}
            ...
            <Fragment key={name}>
              <input
                ...
                onChange={(e: TargetedEvent<HTMLInputElement>) => {
                  onSetLanguage((e.target as HTMLInputElement).value);
                }}
              />
